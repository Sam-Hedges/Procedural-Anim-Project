...
public class ThirdPersonCamera : MonoBehaviour
{
	...
	{		
		...
		float rightX = -_iLook.LookInputVector.x;
		float rightY = _iLook.LookInputVector.y;
		float leftX = 0;
		float leftY = 0;
		float mouseWheel = 0;
		...
		bool qKeyDown = false;
		bool eKeyDown = false;
		bool lShiftKeyDown = false;

		leftTrigger = _input.Player.Aim.GetButtonDown();
		...
		{
			// * First Person *
			if (rightY > firstPersonThreshold && camState != CamStates.Free && follow.GetComponent<Rigidbody>().velocity.magnitude == 0)
			{
			...
			// * Free *
			if ((rightY < freeThreshold || mouseWheel < 0f) && System.Math.Round(follow.GetComponent<Rigidbody>().velocity.magnitude, 2) == 0)
			{
			...
			if ((camState == CamStates.FirstPerson && bButtonPressed) || 
				(camState == CamStates.Target && leftTrigger))
			{
		...
		switch (camState)
		{
			case CamStates.Behind:
                ...
                if (follow.IsGrounded && follow.GetComponent<Rigidbody>().velocity.magnitude != 0)
				{
